...
export class ShoppingListDetailsComponent implements OnInit {
  ...
  offers: any[] = [];
  ingredientOfferMapping: {
    ingredient: any;
    offers: { offer: any; selected: boolean }[];
    status: string;
    selected: boolean;
  }[] = [];
  shoppingListSummary: {
      ...
      .subscribe({
        next: (list) => {
          if (list) {
            ...
            this.ingredientNames = Object.keys(
              this.shoppingList.groupedShoppingList,
            );
  ...
  checkIfShoppinglistHasOngoing() {
    ...
    this.offerToOrderService
      .getByShoppinglistId(this.shoppingList.id)
      .subscribe({
        next: (result) => {
  ...
  private fetchOffers(address: AddressType): void {
    this.offerService
      .setOffersBySearchRadius(this.range, address)
      ...
      .subscribe({
        ...
        error: (error) => {
          this.messageService.add({
            severity: 'error',
            summary: 'Fehler',
            detail: 'Fehler beim Laden der Angebote',
          });
  ...
  getLocalizedLabel(ingredientName: string): string {
    const localizedItem = this.localizationData.find(
  ...
  matchOfferToIngredient(ingredientName: string, offer: any): boolean {
    return (
      ingredientName.toLowerCase() === offer?.ontoFoodType?.label?.toLowerCase()
    );
  }
  getProcessingBreakdown(
    combinedProcessingBreakdown: CombinedProcessingBreakdown,
  ): { label: string; unit: string; amount: number }[] {
    const breakdownArray: { label: string; unit: string; amount: number }[] =
      [];

    for (const [label, unitAmounts] of Object.entries(
      combinedProcessingBreakdown,
    )) {
  ...
  toggleOfferSelection(
    ...
    const ingredientMapping = this.ingredientOfferMapping.find(
      (item) => item.ingredient === ingredientName,
    );
    if (ingredientMapping) {
      // Wenn das Angebot ausgewählt wird, alle anderen Angebote abwählen
      if (isChecked) {
        ingredientMapping.offers.forEach(
          (o) => (o.selected = o.offer.offerDetails.id === offerId),
        );
      } else {
        // Wenn das Angebot abgewählt wird, nur dieses Angebot abwählen
        const offer = ingredientMapping.offers.find(
          (o) => o.offer.offerDetails.id === offerId,
        );
        if (offer) {
      ...
      ingredientMapping.selected = ingredientMapping.offers.some(
        (o) => o.selected,
      );
    }
  }
  ...
  createSummary(): void {
    ...
    this.offerToOrderService
      .createMappedOffersIngredients(shoppingListToOrder)
      .subscribe({
        next: (response) => {
  ...
  getTotalAmountsPerUnit(ingredientName: string): TotalAmountsPerUnit {
    const ingredients = this.shoppingList?.groupedShoppingList[
      ingredientName
    ] as Ingredient[];
  ...
  getTotalAmountsPerUnitArray(
    ingredientName: string,
  ): { unit: string; totalAmount: number }[] {
  ...
  getCombinedProcessingBreakdown(
    ingredientName: string,
  ): CombinedProcessingBreakdown {
    const ingredients = this.shoppingList?.groupedShoppingList[
      ingredientName
  ...
  onOfferSelectionChange(): void {
    const ingredientMapping = this.ingredientOfferMapping.find((mapping) =>
      mapping.ingredient.some(
        (ing: any) => ing.name === this.selectedIngredientName,
      ),
    );
  ...
  getProcessingBreakdownLabelsForIngredient(ingredientName: string): string[] {
    const combinedProcessingBreakdown =
      this.getCombinedProcessingBreakdown(ingredientName);
    const processingBreakdownItems = this.getProcessingBreakdown(
      combinedProcessingBreakdown,
    );
    let labels: string[] = [];
  ...
  getProcessingMatchType(offerItem: any): 'exact' | 'partial' | 'none' {
    ...
    const levelsOfProcessing =
      offerItem.offer?.offerDetails?.levelsOfProcessing ||
    ...
    const processingBreakdownLabels =
      this.getProcessingBreakdownLabelsForIngredient(
        this.selectedIngredientName,
      );

    // Normalize the labels for comparison
    const normalizedIngredientLabels = processingBreakdownLabels.map((label) =>
      label.trim().toUpperCase(),
    );
    const offerProcessingLabels =
      levelsOfProcessing?.map((lp: any) => lp.label.trim().toUpperCase()) || [];
    ...
    if (!ingredientRequiresProcessing) {
      ...
      if (!offerHasProcessing) {
        ...
        const isExactMatch =
          processingBreakdownSet.size === offerProcessingSet.size &&
          [...processingBreakdownSet].every((label) =>
            offerProcessingSet.has(label),
          );
        ...
        const hasPartialMatch = [...processingBreakdownSet].some((label) =>
          offerProcessingSet.has(label),
        );
  ...
  getOffersFoundSelectedCount(): number {
    return this.ingredientOfferMapping.filter(
      (item) => item.status === 'OFFERS_FOUND' && item.selected,
    ).length;
  }
  ...
  getMeterItems(): MeterItem[] {
    ...
    const items = [
      {
        label: 'Ausgewählt',
        value: this.getOffersFoundSelectedCount(),
        icon: 'pi pi-check',
        color: 'green',
        size: (this.getOffersFoundSelectedCount() / total) * 100,
      },
      {
        label: 'Verfügbar',
        value: this.getOffersFoundUnselectedCount(),
        icon: 'pi pi-pencil',
        color: 'orange',
        size: (this.getOffersFoundUnselectedCount() / total) * 100,
      },
      {
        label: 'Unverfügbar',
        value: this.getNoOffersCount(),
        icon: 'pi pi-ban',
        color: 'red',
        size: (this.getNoOffersCount() / total) * 100,
      },
    ];
    return items;
  }
  ...
  hasNoUnselectedOffers(): boolean {
    return this.ingredientOfferMapping.every(
      (item) => item.status !== 'OFFERS_FOUND' || item.selected,
    );
  }
}
